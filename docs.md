...
# ========================================================
# 🏗️ STAGE 1: BUILDER (Збірка залежностей)
# ========================================================
FROM python:3.11-slim AS builder

# Вимикаємо кєш pip та створення .pyc файлів
ENV PYTHONDONTWRITEBYTECODE=1 \
    PYTHONUNBUFFERED=1

WORKDIR /build

# 1. Встановлюємо системні пакети для компіляції (якщо потрібні C-бібліотеки)
RUN apt-get update && apt-get install -y --no-install-recommends \
    build-essential \
    curl \
    && rm -rf /var/lib/apt/lists/*

# 2. Копіюємо requirements.txt та збираємо wheel-пакети у пакувальну папку
COPY requirements.txt .
RUN pip install --no-cache-dir --user -r requirements.txt

# ========================================================
# 🚀 STAGE 2: FINAL RUNTIME (Мінімальний робочий образ)
# ========================================================
...
# 1. Створюємо безпечного не-root користувача appuser
RUN groupadd -g 10001 appgroup && \
    useradd -u 10001 -g appgroup -s /bin/sh -m appuser

WORKDIR /app

# 2. Копіюємо скомпільовані Python бібліотеки з Stage 1
COPY --from=builder /root/.local /home/appuser/.local

# 3. Копіюємо вихідний код мікросервісу
COPY --chown=appuser:appgroup . /app

# 4. Перемикаємося на не-root користувача для безпеки
USER appuser

# 5. Відкриваємо порт мікросервісу
EXPOSE 8000

# 6. Перевірка життєздатності (Healthcheck)
HEALTHCHECK --interval=30s --timeout=5s --start-period=5s --retries=3 \
  CMD curl -f http://localhost:8000/health || exit 1

# 7. Запуск через Uvicorn (FastAPI)
