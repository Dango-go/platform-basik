1. 🚀 POST /api/v1/databases — Створення нової бази (Provisioning)
...
2. 📋 GET /api/v1/databases — Список активних баз (Installed Database List)
...
3. 🔍 GET /api/v1/databases/{id} — Повний паспорт конкретної бази
...
4. 📈 PATCH /api/v1/databases/{id}/scale — Day-2 Scaling (Масштабування на льоту)
...
5. ⚙️ PUT /api/v1/databases/{id}/config — Parameter Editing (Тюнінг конфігів)
...
6. ⏹️ POST /api/v1/databases/{id}/stop & /start — Призупинка та Запуск (Pause/Resume)
...
7. 🗑️ DELETE /api/v1/databases/{id} — Safe Deletion (Безпечне видалення)
